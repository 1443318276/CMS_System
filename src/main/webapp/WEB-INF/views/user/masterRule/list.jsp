    ...
    <div class="content">
        <div class="card">
            <section class="card-header">
                <h3 class="card-title">管理员权限配置</h3>
            ...
            <section class="card-body">
                <div id="toolbar" class="input-group">
                    ...
                    <select class="form-control" name="masterId">
                        <c:forEach items="${masterList}" var="i">
                            <option value="${i.id}">${i.masterId}</option>
                        </c:forEach>
                    </select>
                </div>
    ...
    ($ => {
        ...
        $(() => {
            table.bootstrapTable({
                url: "master/userMasterRule.do",
                queryParams(params) {
                    return JSON.stringify($.extend({}, params, {
                        extras: {
                            routerId: parseInt($("#toolbar select[name='masterId']").val())
                        } // 附加的额外请求参数
                    }));
                },
                columns: [{   //代表一列的数据
                    ...
                    field: "ruleState", // 数据对象的属性名称，当前列展示当前属性的值
                    title: "权限状态",//当前列的标题名称
                    align: "center", //left center  right 单元格文字的对齐方式
            ...
            $("#toolbar").on("click", "button", function (e) {
                ...
                if (target.hasClass("add-item")) {
                    ...
                    $.peanutPage.modal({
                        title: "新增管理员权限信息",
                        url: "master/userMasterRuleEdit.do"
                    });
                } else if (target.hasClass("del-checked")) {
                    //删除所选项
                    $.peanutPage.delElement({
                        title: "删除警告",
                        text: "是否确认删除当前选中的管理员权限",
                        url: "master/delUserMasterRule.do",
                        table: "#bootstrapTable"
                    });
                }
            });
        ...
        window.actionEvents = {
            "click .edit": function (e, value, row, index) {
                ...
                $.peanutPage.modal({
                    title: "修改管理员权限信息",
                    url: "master/userMasterRuleEdit.do?id=" + row.id
                });
            },
